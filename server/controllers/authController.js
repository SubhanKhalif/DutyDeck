...
/**
 * Checks mailer env vars for debugging
 */
function checkMailerEnvVars() {
  if (!process.env.EMAIL) {
    throw new Error("EMAIL environment variable is not set.");
  }
  if (!process.env.EMAIL_PASSWORD && !process.env.SMTP_PASSWORD && !process.env.MAILER_PASS) {
    throw new Error("Email transport credentials are missing in environment variables.");
  }
}
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    // Check that mailing config is present
    try {
      checkMailerEnvVars();
    } catch (envErr) {
    ...
    // For security, ensure expiry is 10 minutes (not 1 hour as before)
    ...
    try {
      ...
      // Extra debugging for Render or SMTP issues
      let errMsg = emailError && emailError.toString ? emailError.toString() : String(emailError);
      console.error("Error sending email (forgotPassword):", errMsg, emailError);
      return res.status(500).json({
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    if (user.resetPasswordOTP?.toString().toLowerCase() !== otp.toString().toLowerCase()) {
